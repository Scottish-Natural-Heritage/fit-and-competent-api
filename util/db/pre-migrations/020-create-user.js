...
if (process.env.NODE_ENV === 'production') {
  ...
  module.exports = {
    up: (queryInterface, Sequelize) => {
      return queryInterface.sequelize.query('create role fc with noinherit login password :fcPassword;', {
        type: Sequelize.QueryTypes.RAW,
        replacements: {
          fcPassword: config.password
        }
      });
    },
    down: (queryInterface, Sequelize) => {
      return queryInterface.sequelize.query('drop role fc;', {
        type: Sequelize.QueryTypes.RAW
      });
    }
  };
